...
export default function CryptoLiquidityPage() {
  ...
  const cards = [["Stablecoin Supply", money(data.stablecoin.current), data.sources.stablecoin ? `30D ${pct(data.stablecoin.changePct30d)}` : "資料來源暫時無法取得", data.sources.stablecoin || "Unavailable"], ["BTC ETF Net Flow", data.etf.status === "available" ? money(data.etf.btcNetFlow) : "Coming soon", data.etf.status === "available" ? "Available" : "資料來源評估中", data.sources.etf || "Coming soon"], ["ETH ETF Net Flow", data.etf.status === "available" ? money(data.etf.ethNetFlow) : "Coming soon", data.etf.status === "available" ? "Available" : "資料來源評估中", data.sources.etf || "Coming soon"], ["BTC Market Cap", money(data.market.totalMarketCap), data.sources.market ? `30D ${pct(data.market.marketCapChangePct)}` : "資料來源暫時無法取得", data.sources.market || "Unavailable"], ["BTC Spot Volume", money(data.market.totalVolume), data.sources.market ? (data.market.volumeRatio30d == null ? "30D baseline unavailable" : `${data.market.volumeRatio30d}x previous 30D avg`) : "資料來源暫時無法取得", data.sources.market || "Unavailable"]];
  return <main className="analysis-page"><div className="analysis-header"><div><p className="text-sm font-medium uppercase tracking-widest text-blue-600">Macro liquidity</p><h1 className="mt-2 text-3xl font-bold">Crypto Liquidity</h1><p className="mt-2 max-w-2xl text-gray-600">追蹤 stablecoin 供應、BTC 市值與成交量，判斷資金是否正在進入 Crypto 生態系。</p></div><div className={`rounded-lg border px-4 py-3 text-right ${biasClass}`}><div className="text-xs">Liquidity Bias</div><div className="text-lg font-semibold">{biasLabel(data.liquidityBias)}</div><div className="text-xs opacity-70">As of {new Date(data.asOf).toLocaleString()}</div></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">{cards.map(([title, value, change, source]) => <div key={title} className="analysis-panel"><div className="text-sm text-gray-500">{title}</div><div className="mt-3 text-2xl font-bold">{value}</div><div className="mt-2 text-sm text-gray-600">{change}</div><div className="mt-4 text-xs text-gray-400">Source: {source}</div></div>)}</div><section className="analysis-panel mt-8"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Liquidity trend (indexed to 100)</h2><p className="text-sm text-gray-500">用相對變化比較 stablecoin supply 與 BTC market cap。</p></div><div className="flex gap-1 rounded-lg bg-gray-100 p-1">{[30, 90, 365].map((range) => <button key={range} onClick={() => setDays(range)} className={`rounded-md px-3 py-1 text-sm ${days === range ? "bg-white font-medium shadow-sm" : "text-gray-600"}`}>{range === 365 ? "1Y" : `${range}D`}</button>)}</div></div><div className="mt-5"><IndexedChart stablecoin={data.stablecoin.history} market={data.market.history} days={days} /></div></section><section className="mt-8 grid gap-5 md:grid-cols-2"><div className="analysis-panel"><h2 className="text-lg font-semibold">Bias reasons</h2><ul className="mt-4 list-disc space-y-2 pl-5 text-sm text-gray-600">{data.biasReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></div><div className="analysis-panel"><h2 className="text-lg font-semibold">Data sources & limitations</h2><p className="mt-3 text-sm text-gray-600">Stablecoin supply 使用 DeFiLlama 的 USDT + USDC；BTC 市值與成交量使用 CoinPaprika 免費一年歷史資料。BTC 不是全 Crypto 市值。ETF flow 資料仍在評估可靠來源，不會計入 Bias。</p><p className="mt-3 text-xs text-gray-400">這些是流動性背景指標，不是交易建議。</p></div></section><p className="mt-6 text-sm text-gray-500">想查看完整股票分析？<Link className="ml-1 underline" href="/scanner">前往 VP Scanner</Link></p></main>;
}
