...
- `messages/zh-TW.json`、`messages/en.json`、`src/i18n/request.ts`：Next.js frontend 的 request-scoped localization dictionary；`NEXT_LOCALE` cookie 選擇語言，NextIntl provider 將 messages 提供給 Server/Client Components。API machine values 與分析資料 schema 不隨語言改名。
- `src/app/api/auth/*`、`src/lib/auth.ts`：NextAuth/Supabase authentication。JWT 中的方案是五分鐘 freshness window 的 UI snapshot，一般 session refresh 在 window 內不查 Supabase；production data entitlement 不信任此 snapshot，仍由 route handler 即時查核資料庫。
...
- `supabase_watchlist.sql`：新舊環境共用且唯一的個人觀察清單 canonical migration；新環境先套 `supabase_migration.sql` 與最新版 `supabase_billing_providers.sql`，再套 watchlist migration。Watchlist migration 以 transaction 與 exclusive table lock 套用；先拒絕非法或正規化後重複的 draft ticker，再統一大小寫／空白並正規化排序，最後建立 ticker 格式、非負排序、每 user 唯一且 deferred 的排序 constraint，以及自動 `updated_at` trigger。
...
- `src/lib/rate-limit.ts`、`proxy.ts`：Next.js 16 request proxy，負責 Upstash rate limit 與登入頁面保護。Blacklist 與 rate-limit lookup 並行執行，但保留已知 blacklist `403`、已知超限 `429`，只有無決定性結果的 dependency rejection 才套用 tier failure policy。
...
- `src/app/globals.css` 的 `analysis-page`、`analysis-header`、`analysis-panel` 只封裝 Crypto Liquidity 的 presentation styles，不得用 descendant selector 或全域 layout override 套用至其他分析頁；Scanner、Accumulation、Fusion、Strategy、Indicator 子頁、Liquidity、FVG 與 MACD 保留各自適合圖表／表格的容器與 spacing。分析介面不以 emoji 作為標題、方向或狀態語意，應使用文字、Badge 與色彩；品牌 icon、功能性選單控制與資料圖表不受此限制。
- Scanner、Accumulation、Fusion、Dashboard 與單一標的 route 使用共用 route-level loading skeleton，讓 App Router 可預取 fallback 並在資料頁切換時立即回饋。
...
`/fusion`、`/account` 與 `/dashboard` 由 proxy 要求登入；Dashboard 的方案、ticker universe 與 ownership 另由 route handler 以 server-side DB lookup 強制執行。
