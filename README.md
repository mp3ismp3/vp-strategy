...
MACD／Indicator 與 `macd_scan.py` 使用 MACD 線（EMA12 − EMA26）判斷背離；穿越零值仍分開辨識正負區間，MACD 轉折必須有兩側反向斜率，走平與末端未反轉不視為確認。價格轉折仍需日線 5 根、週線 3 根後續 K 棒，圖上日期是價格轉折日，不是可即時得知的日期。

日線圖表資料新增選用 `daily.captured_at`，由 `export_frontend_data.py` 在下載日線前記錄 UTC 時間。MACD／RVOL 依資料擷取當時的紐約時間判斷完成度，不依瀏覽器目前時間；同日日 K 在 16:00 ET 後才納入，夏／冬令時間自動換算，提早收盤日也保守等至 16:00。舊資料沒有擷取時間時排除最後一根，不會把舊盤中快照在隔日自動升格為完整日 K。週線按週一至週日日曆週聚合，只納入截至擷取時已過週五收盤的週；週一休市不合併兩週，週五休市會保守延後確認。Python MACD 正式掃描也依批次下載前時間排除未完成日／週 K。此完成度規則針對平台美股與 ETF，並非盤中同時段 RVOL，也未支援 24 小時市場。

Indicator 預設分頁顯示「突破與回踩」，原 `/macd` 路徑保留。主畫面先呈現日 K／成交量圖與 RVOL 追蹤表；MACD 為預設收合的選看輔助區，展開後才載入其圖表，日週同向背離不標為最強訊號。判斷規則與 RVOL 分級收合；已到期形態預設隱藏，可勾選顯示，近期失效形態仍保留。一般量價圖沿用已授權 OHLC 的可見範圍，突破位與事件標記僅對付費方案顯示，RVOL 與 MACD 訊號明細各自沿用 Indicator 方案限制。RVOL = 指定已完成日 K 成交量／該日前 20 根日 K 均量（不含該日）；少於 21 根、無效量或均量零顯示資料不足。分級為 `<0.7` 明顯縮量、`0.7–<1` 普通、`1–<1.5` 有量、`1.5–2` 明顯放量、`>2` 異常大量。頁面標示分析日期、均量起訖、資料擷取時間，圖上成交量與均量均對齊同一個日 K 日期。

Indicator「突破與回踩」的「已發生事實與判斷條件」提供 RVOL 自動摘要與逐項比對，原始事實與規則可展開查看；沿用既有付費訊號明細權限。比對最新收盤相對原突破位、突破日放量、原形態觀察期、最新日一般回踩／跌破後收復價格條件，以及一般回踩縮量條件。每列附數據日期、數值與依據，結果分為「成立／未成立／不適用／資料不足」。突破當日、已失效或到期形態不判定新的回踩；價格條件與量能有效性分開，缺量不等於條件未成立。摘要分開原突破、最近歷史事件與最新完成日，不把舊回踩配上最新量能，也不把收復列為一般縮量回踩。日期或紀錄與日 K 不一致時停止產生比對。比較使用原始精度，畫面價格與 RVOL 顯示至小數兩位。此功能只解釋既有規則，不提供勝率、買賣建議或新增評分；FVG、Liquidity Sweep 與 MACD 尚未加入跨指標摘要，避免不同日期／確認時間混用。

Liquidity Sweep 分頁提供獨立的「自動摘要＋逐項比對」，可切換目前篩選範圍內的 Sweep 紀錄，預設最新一筆，沿用既有付費明細權限。摘要分開事件日與最新日，比對穿越、收盤收回、0.05%–3% 穿越幅度、收盤離掃蕩極值的距離占振幅至少 30%、事件日量比至少 1，以及最新收盤是否位於事件收回側；最新日比較不代表期間連續守住或產生新事件。Sweep 量比分母為含事件日的 20 根成交量中位數，與 RVOL 不同；歷史資料不足 20 根、量無效或中位數為零時顯示資料不足。正式 detector 保存水平來源與確認日，只從確認後偵測 Sweep；圖上水平也從確認日開始繪製。若外部舊紀錄缺少確認時間，回看摘要會標示資料不足，不宣稱當時可即時取得。

Liquidity Sweep 的原始型態只列為「事件」，不直接當成交易信號。頁面以固定、未最佳化的規則逐日驗證：事件收盤後於下一交易日開盤進場，第五個交易日收盤出場，扣除來回 0.10% 成本；同日同方向只計一筆，多空衝突及持倉重疊不進場。多空方向分開驗證，每個事件只能使用當時已完成的同方向先前交易；至少 20 筆且平均淨報酬 95% 信賴下限與中位數都大於 0 才顯示「歷史門檻信號」，否則標示樣本不足或未通過。這是固定規則的歷史篩選，不代表未來績效已獲證明。若平均淨報酬、中位數與勝率三者皆偏正但尚未通過門檻，表格與圖上藍色 `WATCH` 另標示「偏正觀察（非信號）」，不產生信號星號；可搭配其他指標的同日期已完成資料人工判讀，但在另行完成跨指標回測前不自動加分。可在 `services/frontend/` 執行 `npm run backtest:liquidity -- --reporter=verbose`，使用 `data/frontend_charts.json` 重現固定規則報告；跨標的彙總僅供描述，不會取代逐標的、分方向的信號 gate。

Liquidity Sweep 單檔依 `daily.captured_at` 排除未完成日 K，缺少擷取時間時保守排除最後一根，圖表、清單與摘要使用同一批完成日資料；摘要不修改偵測門檻、Python 策略、JSON schema，也不合併跨指標訊號或產生買賣評分。

FVG 分頁提供獨立的「自動摘要＋逐項比對」，可切換目前篩選範圍內的缺口紀錄，預設最新一筆，沿用既有付費明細權限。摘要分開結構中間日與第三根完成日，列出缺口區間、最新收盤位置與距離、形成後再次觸及、累計填補比例及首次完全填補日期；形成當日不算再次觸及，已填補缺口不因價格回升而恢復有效，填補判斷使用原始 K 棒精度。缺口清單仍依目前快照 ATR 篩選，第三根完成日不代表當時已通過相同篩選。FVG 圖表與摘要使用同一批已完成日 K，日期或紀錄不一致時停止產生比對；此功能只解釋既有規則，不提供勝率、買賣建議或新增評分。

價格形態明確稱為「突破 20 日高點」，不是 VP 壓力判斷。收盤突破前 20 日最高價後固定原突破位，RVOL `>=1.5` 為突破放量、`<1` 為突破量不足，其餘量能普通。追蹤後續 10 根日 K：未回踩保留等待狀態；觸及突破位上方容許範圍且收盤守住為回踩；最低價跌破下方容許範圍再收回則獨立標記「跌破後收復」，不標為一般縮量回踩。容許範圍在突破日固定為 `min(原突破價 × 0.5%, 突破前14根平均 True Range × 0.25)`，不隨後續波動擴大。回踩 RVOL `<0.7` 為縮量、`>=1.5` 為放量，其餘量能普通。

收盤跌破原突破位即失效，超過 10 根則觀察期結束；終止狀態保留到原突破後第 20 根，之後不再展示，期間若出現新突破可建立新形態。正常追蹤中不因連創新高改變原突破位。事件日與事件 RVOL 一起保存，最新日 RVOL 另列，避免把過去回踩標籤配上今天量能。圖上標出原突破位、突破日與最新事件，表格顯示距原突破位百分比。這些是向上突破／回踩觀察規則，門檻尚未經真實市場績效驗證，不修改既有策略評分或 VP JSON schema；RVOL 未加入 Telegram。

批次 MACD 掃描使用 America/New_York 的實際夏冬令時，不以固定 UTC−4 判斷收盤。
